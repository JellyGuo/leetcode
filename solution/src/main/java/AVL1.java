 ...
 * <p>
 * 作者：失火的夏天
 * 链接：https://leetcode.cn/problems/balance-a-binary-search-tree/solutions/150820/shou-si-avlshu-wo-bu-guan-wo-jiu-shi-yao-xuan-zhua/
 * 来源：力扣（LeetCode）
...
public class AVL1 {
    ...
    private TreeNode delete(TreeNode root, int val, Map<TreeNode, Integer> nodeHeight) {
        if (root == null) return null;
        TreeNode node = root;
        int diff = val - root.val;
        if (diff < 0) {
            node.left = delete(node.left, val, nodeHeight);
            if (nodeHeight.getOrDefault(node.right, 0) - nodeHeight.getOrDefault(node.left, 0) > 1) {
                // 另一边的先平衡
                if (nodeHeight.getOrDefault(node.right.left, 0) > nodeHeight.getOrDefault(node.right.right, 0)) {
                    node.right = rotateRight(node.right, nodeHeight);
                }
                node = rotateLeft(node, nodeHeight);
            }
        } else if (diff > 0) {
            node.right = delete(node.right, val, nodeHeight);
            if (nodeHeight.getOrDefault(node.left, 0) - nodeHeight.getOrDefault(node.right, 0) > 1) {
                if (nodeHeight.getOrDefault(node.left.right, 0) > nodeHeight.getOrDefault(node.left.left, 0)) {
                    node.left = rotateLeft(node.left, nodeHeight);
                }
                node = rotateRight(node, nodeHeight);
            }
            ...
            if (node.left == null || node.right == null) {
                ...
                node.right = delete(node.right, node.val, nodeHeight);
                if (nodeHeight.getOrDefault(node.left, 0) - nodeHeight.getOrDefault(node.right, 0) > 1) {
                    if (nodeHeight.getOrDefault(node.left.right, 0) > nodeHeight.getOrDefault(node.left.left, 0)) {
                        node.left = rotateLeft(node.left, nodeHeight);
                    }
